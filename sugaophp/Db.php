...
class Db {
    ...
    static public function getInstance($config=array()) {
        ...
        if(!isset(self::$instance[$md5])) {
            ...
            $class  =   !empty($options['lite'])?  'sugaophp\Db\Lite' :   'sugaophp\\Db\\Driver\\'.ucwords(strtolower($options['type']));
    ...
    static private function parseConfig($config){
        if(!empty($config)){
            ...
            $config = array (
                'type'          =>  C('DB_CONFIG.DB_TYPE'),
                'username'      =>  C('DB_CONFIG.DB_USER'),
                'password'      =>  C('DB_CONFIG.DB_PWD'),
                'hostname'      =>  C('DB_CONFIG.DB_HOST'),
                'hostport'      =>  C('DB_CONFIG.DB_PORT'),
                'database'      =>  C('DB_CONFIG.DB_NAME'),
                //'dsn'           =>  C('DB_CONFIG.DB_DSN'),
                'params'        =>  C('DB_CONFIG.DB_PARAMS'),
                'charset'       =>  C('DB_CONFIG.DB_CHARSET'),
                'deploy'        =>  C('DB_CONFIG.DB_DEPLOY_TYPE'),
                'rw_separate'   =>  C('DB_CONFIG.DB_RW_SEPARATE'),
                'master_num'    =>  C('DB_CONFIG.DB_MASTER_NUM'),
                'slave_no'      =>  C('DB_CONFIG.DB_SLAVE_NO'),
                'debug'         =>  C('DB_CONFIG.DB_DEBUG'),
                //'debug'         =>  C('DB_CONFIG.DB_DEBUG',null,APP_DEBUG),
                //'lite'          =>  C('DB_CONFIG.DB_LITE'),
            );
        }
        return $config;
    }
