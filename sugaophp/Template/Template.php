...
class Template {
    ...
    public function __construct() {
        $this->config['cache_path'] = $this->sugao_config['CACHE_PATH'];
        $this->config['template_suffix'] = $this->sugao_config['TMPL_TEMPLATE_SUFFIX'];
        $this->config['cache_suffix'] = $this->sugao_config['TMPL_CACHFILE_SUFFIX'];
        $this->config['tmpl_cache'] = $this->sugao_config['TMPL_CACHE_ON'];
        $this->config['cache_time'] = $this->sugao_config['TMPL_CACHE_TIME'];
        $this->config['taglib_begin'] = $this->stripPreg($this->sugao_config['TAGLIB_BEGIN']);
        $this->config['taglib_end'] = $this->stripPreg($this->sugao_config['TAGLIB_END']);
        $this->config['tmpl_begin'] = $this->stripPreg($this->sugao_config['TMPL_L_DELIM']);
        $this->config['tmpl_end'] = $this->stripPreg($this->sugao_config['TMPL_R_DELIM']);
        $this->config['default_tmpl'] = '';
        $this->config['layout_item'] = $this->sugao_config['TMPL_LAYOUT_ITEM'];
    }
    ...
    public function loadTemplate($templateFile, $prefix = '') {
        ...
        // 判断是否启用布局
        if ($this->sugao_config['LAYOUT_ON']) {
            if (false !== strpos($tmplContent, '{__NOLAYOUT__}')) { // 可以单独定义不使用布局
                $tmplContent = str_replace('{__NOLAYOUT__}', '', $tmplContent);
            } else { // 替换布局的主体内容
                $layoutFile = THEME_PATH . $this->sugao_config['LAYOUT_NAME'] . $this->config['template_suffix'];
                // 检查布局文件
    ...
    public function parse($content) {
        ...
        if ($this->sugao_config['TAGLIB_LOAD']) {
        ...
        if ($this->sugao_config['TAGLIB_PRE_LOAD']) {
            $tagLibs = explode(',', $this->sugao_config['TAGLIB_PRE_LOAD']);
            foreach ($tagLibs as $tag) {
                $this->parseTagLib($tag, $content);
            }
        }
        // 内置标签库 无需使用taglib标签导入就可以使用 并且不需使用标签库XML前缀
        $tagLibs = explode(',', $this->sugao_config['TAGLIB_BUILD_IN']);
        foreach ($tagLibs as $tag) {
    ...
    protected function parsePhp($content) {
        ...
        // PHP语法检查
        if ($this->sugao_config['TMPL_DENY_PHP'] && false !== strpos($content, '<?php')) {
            E(L('_NOT_ALLOW_PHP_'));
        }
        return $content;
    }
    ...
    protected function parseLayout($content) {
        ...
        if ($find) {
            ...
            if (!$this->sugao_config['LAYOUT_ON'] || $this->sugao_config['LAYOUT_NAME'] != $array['name']) {
                // 读取布局模板
    ...
    public function parseTagLib($tagLib, &$content, $hide = false) {
        ...
        if (strpos($tagLib, '\\')) {
            ...
            $className = 'sugaophp\\Template\TagLib\\' . ucwords($tagLib);
        }
         $tLib =new $className();
        $that = $this;
    ...
    public function parseTag($tagStr) {
        ...
        // 未识别的标签直接返回
        return $this->sugao_config['TMPL_L_DELIM'] . $tagStr . $this->sugao_config['TMPL_R_DELIM'];
    }
    ...
    public function parseVar($varStr) {
        ...
        if (!empty($varStr)) {
            ...
            if ('Think.' == substr($var, 0, 6)) {
                ...
                switch (strtolower($this->sugao_config['TMPL_VAR_IDENTIFY'])) {
                    case 'array': // 识别为数组
                        $name = '$' . $var;
    ...
    public function parseVarFunction($name, $varArray) {
        ...
        //取得模板禁止使用函数列表
        $template_deny_funs = explode(',', $this->sugao_config['TMPL_DENY_FUNC_LIST']);
        for ($i = 0; $i < $length; $i++) {
